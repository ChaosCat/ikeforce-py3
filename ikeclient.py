...
class IKEv1Client(object):
    ...
    dicDHGroup = {
        ...
        "6": "EC2N group over GF[2^163](see Note)",
        "7": "EC2N group over GF[2^163](see Note)",
        "8": "EC2N group over GF[2^283](see Note)",
        "9": "EC2N group over GF[2^283](see Note)",
        "10": "EC2N group over GF[2^409](see Note)",
        "11": "EC2N group over GF[2^409](see Note)",
        "12": "EC2N group over GF[2^571](see Note)",
        "13": "EC2N group over GF[2^571](see Note)",
    ...
    def payBuild(self, strPayload, lenLen, *arg):
        ...
        try:
            ...
            if arg[0] and lenLen <= 2:
                ...
                payLen = hex(len(wholePayload) / 2 + lenLen)[2:].zfill(
                    lenLen * 2
                )  # +2 for the length payload
                if self.debug > 0:
            ...
            if lenLen <= 2:
                payLen = hex(len(strPayload) / 2 + lenLen)[2:].zfill(
                    lenLen * 2
                )  # +2 for the length payload itself
                if self.debug > 0:
                    print("Calculating length")
                    print("Payload Length:  %s" % payLen)
                ...
                payLen = hex(len(strPayload) / 2 + lenLen)[2:].zfill(
                    lenLen * 2
                )  # +4 for the length payload itself
                if self.debug > 0:
                    print("Calculating length")
                    print("Payload Length:  %s" % payLen)
    ...
    def ikeHeader(
        self, payNext, iCookie, rCookie, version, flags, xType, msgID, payloads
    ):  # Input the payload length to this method for now, need to figure out a calculation for this later
        ...
        if self.debug > 0:
            ...
            print(
                "Exchange Type: %s (%s)" % (xType, self.dicXTypes[str(int(xType, 16))])
            )
            print("Flags: %s" % flags)
    ...
    def ikeProposal(self, strTransforms, payNext, phase):
        ...
        try:
            strPayload = (
                payNext + padding + propNum + protID + spiSize + propTrans + spi
            )
        except:
        ...
        if self.debug > 0:
            ...
            print(
                "Proposal Transforms: %s" % int(propTrans, 16)
            )  # just one transform set for now
            try:
                print("SPI: %s" % spi)
            except:
                pass
    ...
    def ikeNot(self, payNext, msgType, spi, notData):
        ...
        try:
            strPayload = (
                payNext + padding + doi + protID + spiSize + msgType + spi + notData
            )
        except TypeError:
        ...
        if self.debug > 0:
            ...
            try:
                print(
                    "Notify Message Type: %s" % self.dicNotType[str(int(msgType, 16))]
                )
            except:
    ...
    def ikeXAUTH(self, typeXAUTH, attXAUTH, attXAUTHValue, *args):
        ...
        if len(args) > 1:
            print(
                "Only 1 optional argument (vendor) required if responder is Cisco. Exiting..."
            )
            exit()
        try:
        ...
        if attXAUTH == 16527 or attXAUTH == 16520:
            ...
            if self.debug > 0:
                print(
                    "Mode Config Attribute Type: %s" % self.dicXAUTHAtts[str(attXAUTH)]
                )
                print("Mode Config Attribute Value: %s" % attXAUTHValue)
            try:
                payloadXAUTH = hex(attXAUTH)[2:].zfill(4) + hex(attXAUTHValue)[
                    2:
                ].zfill(4)
            except:
        ...
        # Some requests need to have a blank value, to request IP address etc
            ...
            if self.debug > 0:
                try:
                    print(
                        "Mode Config Attribute Type: %s"
                        % self.dicXAUTHAtts[str(attXAUTH)]
                    )
                except:
                    print("Mode Config Attribute Type: %s" % attXAUTH)
                print("Mode Config Attribute Length: %s" % attLen)
    ...
    def ikeModeCFG(self, payNext, mcfgType, mcfgAtts):
        ...
        if self.debug > 0:
            ...
            try:
                ...
                print("Next Payload: %s" % self.dicPayloads[str(payNext)])
            print(
                "Mode Config Message Type: %s"
                % self.dicMCFGType[str(int(mcfgType, 16))]
            )
    ...
    def ikeDelete(self, payNext, iCookie, rCookie):
        ...
        try:
            payNext = hex(payNext)[2:].zfill(2)
            strPayload = (
                payNext + padding + payLen + DOI + protID + sizeSPI + numSPIs + spi
            )
    ...
    def main(
        ...
        if eType == "07" or int(eType) == 7 or eType == "AES":
            arrayTrans = ikeneg.ikeTransform(
                ...
                "00",
                keyLen,
            )  # static lifetime values for now
        else:
            arrayTrans = ikeneg.ikeTransform(
                eType, hashType, authType, DHGroup, "01", "00007080", "01", phase, "00"
            )
        ...
        arrayID, ID_i = ikeneg.ikeID(
            IDdata, idType, "01f4", "11", "0d"
        )  # next payload = none (0), 01f4 = port (500) 0d = vid

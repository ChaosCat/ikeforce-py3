...
try:
    import secrets
    secure_random = secrets.randbits
except Exception:
    try:
        import Crypto.Random.random as _cr

        secure_random = _cr.getrandbits
    except Exception:
        import OpenSSL
        secure_random = lambda x: int(hexlify(OpenSSL.rand.bytes(x >> 3)), 16)
...
class DiffieHellman(object):
    ...
    def __init__(self, DHGroup):
        ...
        if int(DHGroup) == 1:
            ...
            print(
                "Unsupported Diffie-Hellman group specified. Only 1,2 & 5 supported.\nExiting..."
            )
            exit()
